...
public class ChargerController {
    ...
    @GetMapping("/list")
    public Result<Page<Charger>> list(Integer page, Integer limit, String keyword) {
        Page<Charger> pageObj = new Page<>(page, limit);
        if (StringUtils.isNotBlank(keyword)) {
            QueryWrapper<Charger> wrapper = new QueryWrapper<>();
            wrapper.like("device_no", keyword).or().like("manufacturer", keyword);
            return Result.ok(chargerService.page(pageObj, wrapper));
        }
