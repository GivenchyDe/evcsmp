...
public class UserController {
    ...
    @GetMapping("/list")
    public Result<Page<User>> list(Integer page, Integer limit, String keyword) {
        Page<User> pageObj = new Page<>(page, limit);
        if (StringUtils.isNotBlank(keyword)) {
            QueryWrapper<User> wrapper = new QueryWrapper<>();
            wrapper.like("username", keyword).or().like("phone", keyword).or().like("nickname", keyword);
            return Result.ok(userService.page(pageObj, wrapper));
        }
